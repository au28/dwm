...
#define ISVISIBLE(C, M)         ISVISIBLEONTAG(C, M->tagset[M->seltags])
...
static unsigned int sleepinterval = 0, maxinterval = 0, timeinterval = 0;
static unsigned int stsw = 0;
static int screen;
...
attachtop(Client *c)
{
	...
	for (n = 1, below = c->mon->cl->clients;
...
createmon(void)
{
	...
	if (i >= LENGTH(tags))
		for (i=0, tm=mons; tm; tm=tm->next, i++) {
			tm->seltags ^= 1;
			tm->tagset[tm->seltags] = (1<<i) & (~SPTAGMASK);
		}
...
getstatus(int width)
{
	...
	for (i = LENGTH(blocks) - 1; i >= 0; i--)
	{
		...
		/* draw delimiter */
		if (delimiter[0] == '\0') /* ignore no delimiter */
			continue;
		all -= delimlen;
    ...
    while (running) {
        ...
        for (i = 0; i < LENGTH(blocks); i++) {
            if (fds[i + 1].revents & POLLIN) {
                /* empty buffer with CMDLEN + 1 byte for the null terminator */
...
{
    isalarm = 1;
    getcmds(timeinterval);
    alarm(sleepinterval);
    timeinterval = (timeinterval + sleepinterval - 1) % maxinterval + 1;
}
...
toggletag(const Arg *arg)
{
	...
	if (!selmon->sel)
		return;
	newtags = selmon->sel->tags ^ (arg->ui & (~SPTAGMASK));
	if (newtags) {
...
{
	Monitor *m;
	unsigned int newtagset = selmon->tagset[selmon->seltags] ^ (arg->ui & (~SPTAGMASK));
